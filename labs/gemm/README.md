...
- `sgemm_benchmark_lab.cpp`：SGEMM 专项 benchmark 工具，对比 `cuda_naive`、`tiled_gemm_block`、`cublas_sgemm`，后续继续追加 `tiled_gemm_v2` 等。
...
```powershell
out\build\windows-vs2022-cuda-release\labs\gemm\Release\sgemm_benchmark_lab.exe --gemm-m 4096 --gemm-n 4096 --gemm-k 4096 --warmup 2 --iters 5 --gemm-reg-m 4 --gemm-reg-n 4
```

需要观察端到端成本时，显式打开 e2e：

```powershell
out\build\windows-vs2022-cuda-release\labs\gemm\Release\sgemm_benchmark_lab.exe --gemm-m 4096 --gemm-n 4096 --gemm-k 4096 --include-e2e --warmup 2 --iters 5 --gemm-reg-m 4 --gemm-reg-n 4
```

## Profiling

GEMM 的 Nsight Systems / Nsight Compute 分析命令集中维护在 [GEMM Profiling Commands](../../docs/profiling/gemm/README.md)。建议先用 `nsys` 看 timeline 和 NVTX 区间，再用 `ncu` 分别抓 `cuda_naive`、`tiled_gemm_block` 和 `cublas_sgemm` 的 kernel 指标。
...
默认是 `16x16x16`。当前每个维度支持 `8`、`16`、`32`。这些值会传给 `GemmLabTileConfig`，`tiled_gemm_block` 再 dispatch 到对应的模板 kernel 实例，所以 `BLOCK_M`、`BLOCK_N`、`BLOCK_K` 仍然是编译期常量，shared memory 数组尺寸和 `#pragma unroll` 都能保持编译期展开。

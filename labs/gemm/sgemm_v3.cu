...
constexpr bool kSgemmV3KernelImplemented = true;
constexpr int kMaxSgemmV3ThreadsPerBlock = 32 * 32;
constexpr int kFloat4Width = 4;

template <int BlockM, int BlockN, int RegisterM, int RegisterN>
constexpr int sgemm_v3_threads_per_cta() {
    return (BlockM / RegisterM) * (BlockN / RegisterN);
}

template <int BlockM, int BlockN, int RegisterM, int RegisterN>
constexpr bool sgemm_v3_uses_warp_quadrants() {
    return RegisterM == 8 && RegisterN == 8 && BlockM % 32 == 0 && BlockN % 64 == 0;
}
...
__device__ __forceinline__ void load_global_tiles_float4(
    ...
    static_assert(BlockK % kFloat4Width == 0, "sgemm_v3 requires block_k to be divisible by 4.");
    static_assert(BlockN % kFloat4Width == 0, "sgemm_v3 requires block_n to be divisible by 4.");
...
__device__ __forceinline__ void load_register_fragments(
    ...
    if constexpr(sgemm_v3_uses_warp_quadrants<BlockM, BlockN, RegisterM, RegisterN>()) {
        ...
        const int warp_n = warp_id % kWarpsPerNGroup;
        const int a_tile_index = warp_m * 16 + (lane_id / 8) * 4;
        const int b_tile_index = warp_n * 32 + (lane_id % 8) * 4;

        #pragma unroll
        for(int row_item = 0; row_item < 4; ++row_item) {
            lhs_fragment[register_buffer][row_item] =
                lhs_tiles[lhs_tile_offset<BlockM, BlockK>(shared_buffer, a_tile_index + row_item, inner)];
            lhs_fragment[register_buffer][row_item + 4] =
                lhs_tiles[lhs_tile_offset<BlockM, BlockK>(shared_buffer, a_tile_index + kHalfBlockM + row_item, inner)];
        }
        ...
        for(int col_item = 0; col_item < 4; ++col_item) {
            ...
            rhs_fragment[register_buffer][col_item + 4] =
                rhs_tiles[rhs_tile_offset<BlockK, BlockN>(shared_buffer, inner, b_tile_index + kHalfBlockN + col_item)];
        }
    } else {
        #pragma unroll
...
__device__ __forceinline__ void store_output_tile(
    ...
    if constexpr(sgemm_v3_uses_warp_quadrants<BlockM, BlockN, RegisterM, RegisterN>()) {
        ...
        const int c_col_base = warp_n * 32 + (lane_id % 8) * 4;
...
template <int BlockM, int BlockN, int BlockK, int RegisterM, int RegisterN>
__global__ void __launch_bounds__(sgemm_v3_threads_per_cta<BlockM, BlockN, RegisterM, RegisterN>(), 1) sgemm_v3_kernel(
    const float* __restrict__ lhs,
    const float* __restrict__ rhs,
    float* __restrict__ out,
    std::size_t m,
    ...
    static_assert(BlockM == 32 || BlockM == 64 || BlockM == 128, "sgemm_v3 block_m must be 32, 64, or 128.");
    static_assert(BlockN == 32 || BlockN == 64 || BlockN == 128, "sgemm_v3 block_n must be 32, 64, or 128.");
    static_assert(BlockK == 8 || BlockK == 16 || BlockK == 32, "sgemm_v3 block_k must be 8, 16, or 32.");
    static_assert(RegisterM == RegisterN, "sgemm_v3 currently supports square register tiles.");
    static_assert(RegisterM == 4 || RegisterM == 8, "sgemm_v3 register tile must be 4x4 or 8x8.");
    static_assert(BlockM % RegisterM == 0, "sgemm_v3 block_m must be divisible by register_m.");
    static_assert(BlockN % RegisterN == 0, "sgemm_v3 block_n must be divisible by register_n.");

    constexpr int kThreadBlockDimX = BlockN / RegisterN;
    constexpr int kThreadsPerCta = sgemm_v3_threads_per_cta<BlockM, BlockN, RegisterM, RegisterN>();
    static_assert(kThreadsPerCta <= kMaxSgemmV3ThreadsPerBlock, "sgemm_v3 derived block size must fit within 1024.");
...
bool launch_sgemm_v3_instance(
    ...
    if(!attributes_configured) {
        const auto max_dynamic_shared_status = cudaFuncSetAttribute(
            sgemm_v3_kernel<BlockM, BlockN, BlockK, RegisterM, RegisterN>,
...
struct SgemmV3Spec {
    ...
    int register_m;
    int register_n;
    SgemmV3LaunchFn launch;
};

#define AI_SYSTEM_SGEMM_V3_SPEC(BLOCK_M, BLOCK_N, BLOCK_K, REGISTER_M, REGISTER_N) \
    { \
        BLOCK_M, \
        BLOCK_N, \
        BLOCK_K, \
        REGISTER_M, \
        REGISTER_N, \
        &launch_sgemm_v3_instance<BLOCK_M, BLOCK_N, BLOCK_K, REGISTER_M, REGISTER_N> \
    }

#define AI_SYSTEM_SGEMM_V3_REG_PAIRS_FOR_K(BLOCK_M, BLOCK_N, BLOCK_K) \
    AI_SYSTEM_SGEMM_V3_SPEC(BLOCK_M, BLOCK_N, BLOCK_K, 4, 4), \
    AI_SYSTEM_SGEMM_V3_SPEC(BLOCK_M, BLOCK_N, BLOCK_K, 8, 8)

#define AI_SYSTEM_SGEMM_V3_REG_PAIRS(BLOCK_M, BLOCK_N) \
    AI_SYSTEM_SGEMM_V3_REG_PAIRS_FOR_K(BLOCK_M, BLOCK_N, 8), \
    AI_SYSTEM_SGEMM_V3_REG_PAIRS_FOR_K(BLOCK_M, BLOCK_N, 16), \
    AI_SYSTEM_SGEMM_V3_REG_PAIRS_FOR_K(BLOCK_M, BLOCK_N, 32)

#define AI_SYSTEM_SGEMM_V3_N_VALUES(BLOCK_M) \
    AI_SYSTEM_SGEMM_V3_REG_PAIRS(BLOCK_M, 32), \
    AI_SYSTEM_SGEMM_V3_REG_PAIRS(BLOCK_M, 64), \
    AI_SYSTEM_SGEMM_V3_REG_PAIRS(BLOCK_M, 128)

constexpr SgemmV3Spec kSgemmV3Specs[] = {
    AI_SYSTEM_SGEMM_V3_N_VALUES(32),
    AI_SYSTEM_SGEMM_V3_N_VALUES(64),
    AI_SYSTEM_SGEMM_V3_N_VALUES(128)
};

#undef AI_SYSTEM_SGEMM_V3_N_VALUES
#undef AI_SYSTEM_SGEMM_V3_REG_PAIRS
#undef AI_SYSTEM_SGEMM_V3_REG_PAIRS_FOR_K
#undef AI_SYSTEM_SGEMM_V3_SPEC

bool dispatch_sgemm_v3(
    const float* lhs,
    const float* rhs,
    float* out,
    std::size_t m,
    ...
    for(const auto& spec : kSgemmV3Specs) {
    ...
    error = "sgemm_v3 shape is not compiled into the launcher table: block=" +
...
bool launch_sgemm_v3(
    ...
    const ai_system::profiling::ScopedNvtxRange launch_range("sgemm_v3_kernel_launch");

    if(!kSgemmV3KernelImplemented) {
        error = "sgemm_v3 kernel is not implemented yet.";
        return false;
    }

    return dispatch_sgemm_v3(lhs, rhs, out, m, n, k, tile_config, error);
}

bool is_sgemm_v3_kernel_implemented() {
    return kSgemmV3KernelImplemented;
}

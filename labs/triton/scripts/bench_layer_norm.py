...
from triton_playground.kernels.layer_norm import default_group_size_m, default_num_warps, resolve_block_size
...
def assert_layer_norm_close(actual: torch.Tensor, expected: torch.Tensor) -> None:
...
def make_inputs(rows: int, cols: int, dtype: torch.dtype, eps: float) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    bias = torch.rand((cols,), device="cuda", dtype=dtype, requires_grad=True)
    dy = 0.1 * torch.randn_like(x)

    y_tri = layer_norm(x, weight, bias, eps=eps)
    y_ref = F.layer_norm(x, (cols,), weight, bias, eps=eps)
    assert_layer_norm_close(y_tri, y_ref)
    ...
    assert_layer_norm_close(dx_tri, x.grad)
    assert_layer_norm_close(dw_tri, weight.grad)
    assert_layer_norm_close(db_tri, bias.grad)
    x.grad = None
    weight.grad = None
    bias.grad = None
...
def forward_provider(provider: str, x: torch.Tensor, weight: torch.Tensor, bias: torch.Tensor, eps: float) -> torch.Tensor:
    if provider == "triton":
...
def benchmark_provider(
    ...
    if mode == "forward":
        return do_bench_ms(lambda: forward_provider(provider, x, weight, bias, eps), warmup, iters)
    if mode == "backward":
        y = forward_provider(provider, x, weight, bias, eps)
        return do_bench_ms(
...
def estimated_bytes(mode: str, x: torch.Tensor) -> int:
    factor = 2 if mode == "forward" else 3
...
def result_row(
    ...
    if provider == "triton":
        ...
        notes = "Triton affine LayerNorm over the last dimension; backward uses partial dw/db reduction"
    else:
        config = f"mode={mode};torch_layer_norm"
        notes = "torch.nn.functional.layer_norm baseline"
...
def run_single(args: argparse.Namespace, cols: int) -> None:
    ...
    x, weight, bias, dy = make_inputs(args.rows, cols, dtype, args.eps)
...
def run_perf_report(args: argparse.Namespace) -> None:
    ...
    @triton.testing.perf_report(
        triton.testing.Benchmark(
            ...
            ylabel="GB/s_est",
            plot_name=f"w12-layer-norm-{args.mode}",
            args={
                "rows": args.rows,
    ...
    def benchmark(cols: int, provider: str, rows: int, dtype_name: str, mode: str, eps: float, warmup: int, iters: int):
        ...
        x, weight, bias, dy = make_inputs(rows, cols, dtype, eps)
        median_ms, low_ms, high_ms = benchmark_provider(provider, mode, x, weight, bias, dy, eps, warmup, iters)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Benchmark Triton LayerNorm forward/backward against PyTorch.")
    ...
    parser.add_argument("--providers", type=parse_providers, default=parse_providers("triton,torch"))
    parser.add_argument("--warmup", type=int, default=10)
    parser.add_argument("--iters", type=int, default=50)
    parser.add_argument("--output", type=Path, default=OUT_BENCHMARK_ROOT / "w12_layer_norm.csv")

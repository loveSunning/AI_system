...
W12 的目标不是追一个单点性能数字，而是把常见 row-wise / element-wise fused op 做成可测试、可 benchmark、可复盘的 Triton 小体系。当前阶段包含三个算子：
...
- reference: Dropout 使用显式 mask / deterministic seed 语义检查；LayerNorm 对齐 `torch.nn.functional.layer_norm`；RMSNorm 对齐 PyTorch expression `x * rsqrt(mean(x^2) + eps) * weight`。

...
def require_triton_fused_softmax():
    ...
    return torch, fused_softmax, naive_softmax
...
def test_fused_softmax_matches_torch(shape: tuple[int, int], dtype_name: str) -> None:
    torch, fused_softmax, _ = require_triton_fused_softmax()
...
def test_naive_softmax_matches_torch() -> None:
    torch, _, naive_softmax = require_triton_fused_softmax()
    x = torch.randn((17, 129), device="cuda", dtype=torch.float32)

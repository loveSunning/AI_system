from __future__ import annotations

import pytest


def require_triton_rms_norm():
    torch = pytest.importorskip("torch")
    pytest.importorskip("triton")
    if not torch.cuda.is_available():
        pytest.skip("CUDA is required for Triton tests")

    from triton_playground.ops import naive_rms_norm, rms_norm

    return torch, rms_norm, naive_rms_norm


def torch_rms_norm(torch, x, weight, eps: float):
    x_float = x.float()
    weight_float = weight.float()
    rstd = torch.rsqrt(x_float.pow(2).mean(dim=-1, keepdim=True) + eps)
    return (x_float * rstd * weight_float).to(dtype=x.dtype)


def assert_rms_norm_close(torch, actual, expected) -> None:
    if actual.dtype == torch.float16:
        torch.testing.assert_close(actual, expected, rtol=3e-2, atol=3e-2)
    else:
        torch.testing.assert_close(actual, expected, rtol=1e-4, atol=1e-4)


def run_rms_norm_case(
    shape: tuple[int, ...],
    dtype_name: str,
    block_size: int | None = None,
    num_warps: int | None = None,
) -> None:
    torch, rms_norm, naive_rms_norm = require_triton_rms_norm()
    dtype = getattr(torch, dtype_name)
    torch.manual_seed(0)

    x = (-2.3 + 0.5 * torch.randn(shape, device="cuda", dtype=dtype)).requires_grad_(True)
    weight = torch.rand((shape[-1],), device="cuda", dtype=dtype, requires_grad=True)
    dy = 0.1 * torch.randn_like(x)

    y_prod = rms_norm(x, weight, eps=1e-6, block_size=block_size, num_warps=num_warps)
    y_naive = naive_rms_norm(x, weight, eps=1e-6, block_size=block_size, num_warps=num_warps)
    y_ref = torch_rms_norm(torch, x, weight, eps=1e-6)

    y_prod.backward(dy, retain_graph=True)
    dx_prod = x.grad.detach().clone()
    dw_prod = weight.grad.detach().clone()
    x.grad = None
    weight.grad = None

    y_naive.backward(dy, retain_graph=True)
    dx_naive = x.grad.detach().clone()
    dw_naive = weight.grad.detach().clone()
    x.grad = None
    weight.grad = None

    y_ref.backward(dy, retain_graph=True)
    dx_ref = x.grad.detach().clone()
    dw_ref = weight.grad.detach().clone()

    assert_rms_norm_close(torch, y_prod, y_ref)
    assert_rms_norm_close(torch, y_naive, y_ref)
    assert_rms_norm_close(torch, dx_prod, dx_ref)
    assert_rms_norm_close(torch, dx_naive, dx_ref)
    assert_rms_norm_close(torch, dw_prod, dw_ref)
    assert_rms_norm_close(torch, dw_naive, dw_ref)


@pytest.mark.parametrize("shape", [(1, 1), (2, 7), (17, 129), (4, 3, 257)])
@pytest.mark.parametrize("dtype_name", ["float32", "float16"])
def test_rms_norm_forward_backward_matches_torch(shape: tuple[int, ...], dtype_name: str) -> None:
    run_rms_norm_case(shape, dtype_name)


def test_rms_norm_supports_explicit_block_size() -> None:
    run_rms_norm_case((8, 257), "float32", block_size=512, num_warps=4)


def test_rms_norm_rejects_bad_weight_shape() -> None:
    torch, rms_norm, _ = require_triton_rms_norm()
    x = torch.randn((4, 8), device="cuda", dtype=torch.float32)
    weight = torch.ones((7,), device="cuda", dtype=torch.float32)

    with pytest.raises(ValueError, match="weight must have shape"):
        rms_norm(x, weight)


def test_rms_norm_rejects_non_contiguous_input() -> None:
    torch, rms_norm, _ = require_triton_rms_norm()
    x = torch.randn((8, 16), device="cuda", dtype=torch.float32).t()
    weight = torch.ones((8,), device="cuda", dtype=torch.float32)

    with pytest.raises(ValueError, match="contiguous"):
        rms_norm(x, weight)


def test_rms_norm_rejects_feature_dim_over_64kb() -> None:
    torch, rms_norm, _ = require_triton_rms_norm()
    n_cols = 16385
    x = torch.empty((1, n_cols), device="cuda", dtype=torch.float32)
    weight = torch.empty((n_cols,), device="cuda", dtype=torch.float32)

    with pytest.raises(ValueError, match="64KB"):
        rms_norm(x, weight)

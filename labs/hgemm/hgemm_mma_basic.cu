...
template <int ColStride = 16, int Step = 8>
__device__ __forceinline__ int hgemm_swizzle_permuted_j(int row, int col) {
    static_assert(ColStride <= 16, "HGEMM shared-memory swizzle expects ColStride <= 16.");
    static_assert(Step == 4 || Step == 8, "HGEMM shared-memory swizzle step must be 4 or 8.");
    static_assert(ColStride % Step == 0, "HGEMM shared-memory swizzle expects ColStride to be a multiple of Step.");
    if constexpr(Step == 8) {
        return (((col >> 3) ^ (row >> 2)) % (ColStride >> 3)) << 3;
    } else {
        return (((col >> 2) ^ (row >> 2)) % (ColStride >> 2)) << 2;
    }
}

template <int MmaAtomK = 16>
__device__ __forceinline__ int hgemm_swizzle_a_col(int row, int col) {
    return hgemm_swizzle_permuted_j<MmaAtomK, 8>(row, col);
}

template <int MmaAtomK = 16>
__device__ __forceinline__ int hgemm_swizzle_b_col(int row, int col) {
    return hgemm_swizzle_permuted_j<MmaAtomK, 8>(row, col);
}
...
hgemm_mma_m16n8k16_mma2x4_warp4x4_stages_dsmem_tn_kernel(
    ...
    const int load_smem_a_store_k = SMEM_SWIZZLE ? hgemm_swizzle_a_col<MMA_K>(load_smem_a_m, load_smem_a_k) : load_smem_a_k;
    const int load_smem_b_store_k = SMEM_SWIZZLE ? hgemm_swizzle_b_col<MMA_K>(load_smem_b_n, load_smem_b_k) : load_smem_b_k;
...
#pragma unroll
    for(int k_tile = 0; k_tile < K_STAGE - 1; ++k_tile) {
        ...
        const uint32_t load_smem_a_ptr = smem_a_base_ptr +
            (k_tile * s_a_stage_offset + load_smem_a_m * (BK + A_PAD) + load_smem_a_store_k) * sizeof(half);
        const uint32_t load_smem_b_ptr = smem_b_base_ptr +
            (k_tile * s_b_stage_offset + load_smem_b_n * (BK + B_PAD) + load_smem_b_store_k) * sizeof(half);
...
#pragma unroll
    for(int k_tile = K_STAGE - 1; k_tile < num_k_tiles; ++k_tile) {
        ...
        const int load_gmem_b_k = k_tile * BK + load_smem_b_k;
        const uint32_t load_smem_a_ptr = smem_a_base_ptr +
            (smem_sel_next * s_a_stage_offset + load_smem_a_m * (BK + A_PAD) + load_smem_a_store_k) *
                sizeof(half);
        const uint32_t load_smem_b_ptr = smem_b_base_ptr +
            (smem_sel_next * s_b_stage_offset + load_smem_b_n * (BK + B_PAD) + load_smem_b_store_k) *
                sizeof(half);
        CP_ASYNC_CG(load_smem_a_ptr, &A[load_gmem_a_m * K + load_gmem_a_k], 16);
        CP_ASYNC_CG(load_smem_b_ptr, &B[load_gmem_b_n * K + load_gmem_b_k], 16);
        CP_ASYNC_COMMIT_GROUP();
...
#pragma unroll
        for(int i = 0; i < WARP_TILE_M; ++i) {
            ...
            const int lane_smem_a_load_k = SMEM_SWIZZLE ? hgemm_swizzle_a_col<MMA_K>(lane_smem_a_m, lane_smem_a_k) : lane_smem_a_k;
            const uint32_t lane_smem_a_ptr = smem_a_base_ptr +
                (smem_sel * s_a_stage_offset + lane_smem_a_m * (BK + A_PAD) + lane_smem_a_load_k) *
                    sizeof(half);
            LDMATRIX_X4(RA[i][0], RA[i][1], RA[i][2], RA[i][3], lane_smem_a_ptr);
        }
...
#pragma unroll
        for(int j = 0; j < WARP_TILE_N; ++j) {
            ...
            const int lane_smem_b_load_k = SMEM_SWIZZLE ? hgemm_swizzle_b_col<MMA_K>(lane_smem_b_n, lane_smem_b_k) : lane_smem_b_k;
            const uint32_t lane_smem_b_ptr = smem_b_base_ptr +
                (smem_sel * s_b_stage_offset + lane_smem_b_n * (BK + B_PAD) + lane_smem_b_load_k) *
                    sizeof(half);
...
#pragma unroll
        for(int i = 0; i < WARP_TILE_M; ++i) {
            ...
            const int lane_smem_a_load_k = SMEM_SWIZZLE ? hgemm_swizzle_a_col<MMA_K>(lane_smem_a_m, lane_smem_a_k) : lane_smem_a_k;
            const uint32_t lane_smem_a_ptr = smem_a_base_ptr +
                (stage_sel * s_a_stage_offset + lane_smem_a_m * (BK + A_PAD) + lane_smem_a_load_k) *
                    sizeof(half);
            LDMATRIX_X4(RA[i][0], RA[i][1], RA[i][2], RA[i][3], lane_smem_a_ptr);
        }
...
#pragma unroll
        for(int j = 0; j < WARP_TILE_N; ++j) {
            ...
            const int lane_smem_b_load_k = SMEM_SWIZZLE ? hgemm_swizzle_b_col<MMA_K>(lane_smem_b_n, lane_smem_b_k) : lane_smem_b_k;
            const uint32_t lane_smem_b_ptr = smem_b_base_ptr +
                (stage_sel * s_b_stage_offset + lane_smem_b_n * (BK + B_PAD) + lane_smem_b_load_k) *
                    sizeof(half);
            LDMATRIX_X2(RB[j][0], RB[j][1], lane_smem_b_ptr);
        }
...
// TN WARP_TILE_K=2 swizzle variants copied from the reference HGEMM swizzle kernels.
// They keep A as [M][K], B as physical [N][K], and compute one 128x128 CTA tile.
template<const int MMA_M=16, 
         const int MMA_N=8, 
         const int MMA_K=16,
         const int MMA_TILE_M=2,
...
hgemm_mma_m16n8k16_mma2x4_warp4x4x2_stages_dsmem_tn_swizzle_x2_kernel(
  ...
  for (int k = 0; k < (K_STAGE - 1); ++k) { // 0, 1
    ...
    uint32_t load_smem_a_mma_k_ptr = (
      ...
      hgemm_swizzle_a_col<MMA_K>(load_smem_a_m, load_smem_a_k)) * sizeof(half)
    );
  ...
  { 
    ...
    for (int i = 0; i < WARP_TILE_M; ++i) {
      ...
      uint32_t lane_smem_a_ptr = (
        ...
        hgemm_swizzle_a_col<MMA_K>(lane_smem_a_m, lane_smem_a_k)) * sizeof(half)
      );
...
template<const int MMA_M, 
         const int MMA_N, 
         const int MMA_K,
         const int MMA_TILE_M,
         const int MMA_TILE_N,
         const int WARP_TILE_M,
         const int WARP_TILE_N,
         const int WARP_TILE_K,
         const int A_PAD, 
         const int B_PAD,
         const int K_STAGE, 
         const bool BLOCK_SWIZZLE,
...
hgemm_mma_m16n8k16_mma2x4_warp4x4x2_stages_dsmem_tn_swizzle_x4_kernel(
  ...
  static_assert(WARP_TILE_M == 4 && WARP_TILE_N == 4 && WARP_TILE_K == 2);
  ...
  for (int k = 0; k < (K_STAGE - 1); ++k) { // 0, 1
    ...
    if constexpr (B_PAD == 0) {
      uint32_t load_smem_b_ptr = (
        smem_b_base_ptr + 
        (k * s_b_stage_offset + load_smem_b_n * (BK + B_PAD) + 
        hgemm_swizzle_b_col<MMA_K>(load_smem_b_n, load_smem_b_k)) * sizeof(half)
      );
      CP_ASYNC_CG(load_smem_b_ptr, &B[load_gmem_b_addr], 16);
    ...
    for (int j = 0; j < WARP_TILE_N; ++j) {
      ...
      if constexpr (B_PAD == 0) {
        uint32_t lane_smem_b_ptr = (
          ...
          (0 * s_b_stage_offset + lane_smem_b_n * (BK + B_PAD) + 
          hgemm_swizzle_b_col<MMA_K>(lane_smem_b_n, lane_smem_b_k)) * sizeof(half)
        );
...
hgemm_mma_m16n8k16_mma2x4_warp4x4x2_stages_dsmem_kernel(
  ...
  int load_smem_a_m = tid / 2; // row 0~127
  int load_smem_a_k = (tid % 2 == 0) ? 0 : 8; // col 0,8
  int load_smem_a_store_k = A_SWIZZLE ? hgemm_swizzle_a_col<MMA_K>(load_smem_a_m, load_smem_a_k) : load_smem_a_k;
  int load_smem_b_k = tid / 16; // row 0~15
  ...
  for (int k = 0; k < (K_STAGE - 1); ++k) { // 0, 1
    ...
    uint32_t load_smem_a_ptr = (
      smem_a_base_ptr + (k * s_a_stage_offset + 
                         load_smem_a_m * (BK + A_PAD) + 
                         load_smem_a_store_k) * sizeof(half)
    );
    CP_ASYNC_CG(load_smem_a_ptr, &A[load_gmem_a_addr], 16); // MMA_K 0
    uint32_t load_smem_a_mma_k_ptr = (
      smem_a_base_ptr + s_a_mma_k_store_offset * sizeof(half) + 
  ...
  { 
    ...
    for (int i = 0; i < WARP_TILE_M; ++i) {
      ...
      int lane_smem_a_m = warp_smem_a_m + lane_id % 16; // 0~15
      int lane_smem_a_k = (lane_id / 16) * 8; // 0,8
      int lane_smem_a_load_k = A_SWIZZLE ? hgemm_swizzle_a_col<MMA_K>(lane_smem_a_m, lane_smem_a_k) : lane_smem_a_k;
      uint32_t lane_smem_a_ptr = (
        smem_a_base_ptr + 
        (0 * s_a_stage_offset + lane_smem_a_m * (BK + A_PAD) + 
        lane_smem_a_load_k) * sizeof(half)
      );
      LDMATRIX_X4(RA[reg_store_idx][i][0], RA[reg_store_idx][i][1], 
                  RA[reg_store_idx][i][2], RA[reg_store_idx][i][3], 
                  lane_smem_a_ptr);
...
template <int Stages, bool Swizzle, bool SmemSwizzle = false>
bool hgemm_launch_mma_m16n8k16_mma2x4_warp4x4_stages_dsmem_tn(
    ...
    hgemm_mma_m16n8k16_mma2x4_warp4x4_stages_dsmem_tn_kernel<
        ...
        Stages,
        Swizzle,
        SmemSwizzle><<<grid, block, kSharedBytes>>>(a, b, c, m, n, k);
    return ai_system::cuda_utils::check_last_launch(error);
}
...
bool hgemm_launch_mma_m16n8k16_mma2x4_warp4x4x2_stages_dsmem(
    ...
    if constexpr(Kind == HgemmMmaStageDsmemKind::x2 || Kind == HgemmMmaStageDsmemKind::swizzle_x2) {
        if(!ai_system::cuda_utils::check_status(
               cudaFuncSetAttribute(
                    hgemm_mma_m16n8k16_mma2x4_warp4x4x2_stages_dsmem_kernel<
                        kMmaM,
                        ...
                        Swizzle,
                        true,
                        (Kind == HgemmMmaStageDsmemKind::swizzle_x2)>,
                    cudaFuncAttributeMaxDynamicSharedMemorySize,
                    98304
               ),
                Kind == HgemmMmaStageDsmemKind::swizzle_x2 ?
                    "cudaFuncSetAttribute(hgemm_mma_m16n8k16_mma2x4_warp4x4x2_stages_dsmem_swizzle_kernel)" :
        ...
        hgemm_mma_m16n8k16_mma2x4_warp4x4x2_stages_dsmem_kernel<
            ...
            Swizzle,
            true,
            (Kind == HgemmMmaStageDsmemKind::swizzle_x2)><<<grid, block, kSharedBytes>>>(a, b, c, m, n, k);
...
enum class HgemmMmaStageTnSwizzleKind {
    x2,
    x4
};

template <HgemmMmaStageTnSwizzleKind Kind, int Stages, bool Swizzle, int BPad = 8>
bool hgemm_launch_mma_m16n8k16_mma2x4_warp4x4x2_stages_dsmem_tn_swizzle(
    const half* a,
    const half* b,
    half* c,
    int m,
    ...
    enum : int {
        ...
        kThreads = kMmaTileM * kMmaTileN * kWarpSize,
        kSharedBytes = Stages * kBlockM * (kBlockK + kAPad) * kWarpTileK * static_cast<int>(sizeof(half)) +
            Stages * kBlockN * (kBlockK + kBPad) * kWarpTileK * static_cast<int>(sizeof(half))
    };
    const dim3 block(kThreads);
    const dim3 grid = hgemm_mma_stage_grid(m, n, kBlockM, kBlockN, Swizzle, swizzle_stride);

    if constexpr(Kind == HgemmMmaStageTnSwizzleKind::x2) {
        if(!ai_system::cuda_utils::check_status(
               cudaFuncSetAttribute(
                   hgemm_mma_m16n8k16_mma2x4_warp4x4x2_stages_dsmem_tn_swizzle_x2_kernel<
                       kMmaM,
               ...
               "cudaFuncSetAttribute(hgemm_mma_m16n8k16_mma2x4_warp4x4x2_stages_dsmem_tn_swizzle_x2_kernel)",
               error
           )) {
            return false;
        }
        ...
        if(!ai_system::cuda_utils::check_status(
               ...
               "cudaFuncSetAttribute(hgemm_mma_m16n8k16_mma2x4_warp4x4x2_stages_dsmem_tn_swizzle_x4_kernel)",
               error
           )) {
            return false;
        }
        hgemm_mma_m16n8k16_mma2x4_warp4x4x2_stages_dsmem_tn_swizzle_x4_kernel<
            ...
            Stages,
            Swizzle,
            true><<<grid, block, kSharedBytes>>>(a, b, c, m, n, k);
    }
...
template <bool Swizzle, bool SmemSwizzle = false>
bool hgemm_dispatch_mma_stage_dsmem_tn(
    ...
    switch(stages) {
        case 2:
            return hgemm_launch_mma_m16n8k16_mma2x4_warp4x4_stages_dsmem_tn<2, Swizzle, SmemSwizzle>(
                a, b, c, m, n, k, swizzle_stride, error
            );
        case 3:
            return hgemm_launch_mma_m16n8k16_mma2x4_warp4x4_stages_dsmem_tn<3, Swizzle, SmemSwizzle>(
                a, b, c, m, n, k, swizzle_stride, error
            );
        case 4:
            return hgemm_launch_mma_m16n8k16_mma2x4_warp4x4_stages_dsmem_tn<4, Swizzle, SmemSwizzle>(
                a, b, c, m, n, k, swizzle_stride, error
            );
        case 5:
            return hgemm_launch_mma_m16n8k16_mma2x4_warp4x4_stages_dsmem_tn<5, Swizzle, SmemSwizzle>(
                a, b, c, m, n, k, swizzle_stride, error
            );
        default:
            return hgemm_launch_mma_m16n8k16_mma2x4_warp4x4_stages_dsmem_tn<2, Swizzle, SmemSwizzle>(
...
bool hgemm_dispatch_mma_stage_x2_dsmem(
    ...
    if constexpr(Kind == HgemmMmaStageDsmemKind::swizzle_x2) {
        switch(stages) {
            case 2:
                return hgemm_launch_mma_m16n8k16_mma2x4_warp4x4x2_stages_dsmem<Kind, 2, Swizzle, 0, 8>(
                    a, b, c, m, n, k, swizzle_stride, error
                );
            case 3:
                return hgemm_launch_mma_m16n8k16_mma2x4_warp4x4x2_stages_dsmem<Kind, 3, Swizzle, 0, 8>(
                    a, b, c, m, n, k, swizzle_stride, error
                );
            case 4:
                return hgemm_launch_mma_m16n8k16_mma2x4_warp4x4x2_stages_dsmem<Kind, 4, Swizzle, 0, 8>(
                    a, b, c, m, n, k, swizzle_stride, error
                );
            case 5:
                return hgemm_launch_mma_m16n8k16_mma2x4_warp4x4x2_stages_dsmem<Kind, 5, Swizzle, 0, 8>(
                    a, b, c, m, n, k, swizzle_stride, error
                );
            default:
                return hgemm_launch_mma_m16n8k16_mma2x4_warp4x4x2_stages_dsmem<Kind, 2, Swizzle, 0, 8>(
...
template <HgemmMmaStageTnSwizzleKind Kind, bool Swizzle>
bool hgemm_dispatch_mma_stage_dsmem_tn_swizzle(
    const half* a,
    const half* b,
    half* c,
    int m,
    ...
    switch(stages) {
        case 2:
            return hgemm_launch_mma_m16n8k16_mma2x4_warp4x4x2_stages_dsmem_tn_swizzle<Kind, 2, Swizzle>(
                a, b, c, m, n, k, swizzle_stride, error
            );
        case 3:
            return hgemm_launch_mma_m16n8k16_mma2x4_warp4x4x2_stages_dsmem_tn_swizzle<Kind, 3, Swizzle>(
                a, b, c, m, n, k, swizzle_stride, error
            );
        case 4:
            return hgemm_launch_mma_m16n8k16_mma2x4_warp4x4x2_stages_dsmem_tn_swizzle<Kind, 4, Swizzle>(
                a, b, c, m, n, k, swizzle_stride, error
            );
        case 5:
            return hgemm_launch_mma_m16n8k16_mma2x4_warp4x4x2_stages_dsmem_tn_swizzle<Kind, 5, Swizzle, 0>(
                a, b, c, m, n, k, swizzle_stride, error
            );
        default:
            return hgemm_launch_mma_m16n8k16_mma2x4_warp4x4x2_stages_dsmem_tn_swizzle<Kind, 2, Swizzle>(
...
bool hgemm_mma_m16n8k16_mma2x4_warp4x4x2_stages_dsmem_swizzle(
    ...
    if(swizzle) {
        return hgemm_dispatch_mma_stage_x2_dsmem<HgemmMmaStageDsmemKind::swizzle_x2, true>(
            a, b, c, m, n, k, stages, swizzle_stride, error
        );
    }
    return hgemm_dispatch_mma_stage_x2_dsmem<HgemmMmaStageDsmemKind::swizzle_x2, false>(
        a, b, c, m, n, k, stages, swizzle_stride, error
    );
}
...
bool hgemm_mma_m16n8k16_mma2x4_warp4x4_stages_dsmem_tn_swizzle(
    ...
    const ai_system::profiling::ScopedNvtxRange launch_range("hgemm_mma_m16n8k16_mma2x4_warp4x4_stages_dsmem_tn_swizzle_kernel_launch");
    if(!validate_stage_options(stages, swizzle, swizzle_stride, error) || !validate_device_problem(a, b, c, m, n, k, error)) {
        return false;
    }
    if(swizzle) {
        return hgemm_dispatch_mma_stage_dsmem_tn<true, true>(a, b, c, m, n, k, stages, swizzle_stride, error);
    }
    return hgemm_dispatch_mma_stage_dsmem_tn<false, true>(a, b, c, m, n, k, stages, swizzle_stride, error);
}
...
bool hgemm_mma_m16n8k16_mma2x4_warp4x4x2_stages_dsmem_tn_swizzle_x2(
    ...
    const ai_system::profiling::ScopedNvtxRange launch_range("hgemm_mma_m16n8k16_mma2x4_warp4x4x2_stages_dsmem_tn_swizzle_x2_kernel_launch");
    if(!validate_stage_options(stages, swizzle, swizzle_stride, error) || !validate_device_problem(a, b, c, m, n, k, error)) {
        return false;
    }
    if(swizzle) {
        return hgemm_dispatch_mma_stage_dsmem_tn_swizzle<HgemmMmaStageTnSwizzleKind::x2, true>(
            a, b, c, m, n, k, stages, swizzle_stride, error
        );
    }
    return hgemm_dispatch_mma_stage_dsmem_tn_swizzle<HgemmMmaStageTnSwizzleKind::x2, false>(
        a, b, c, m, n, k, stages, swizzle_stride, error
    );
}
...
bool hgemm_mma_m16n8k16_mma2x4_warp4x4x2_stages_dsmem_tn_swizzle_x4(
    ...
    if(swizzle) {
        return hgemm_dispatch_mma_stage_dsmem_tn_swizzle<HgemmMmaStageTnSwizzleKind::x4, true>(
            a, b, c, m, n, k, stages, swizzle_stride, error
        );
    }
    return hgemm_dispatch_mma_stage_dsmem_tn_swizzle<HgemmMmaStageTnSwizzleKind::x4, false>(
        a, b, c, m, n, k, stages, swizzle_stride, error
    );
}

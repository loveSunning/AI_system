...
$cutlassHeader = if ($CutlassRoot) { Join-Path $CutlassRoot "include/cutlass/cutlass.h" } else { "" }
$cuteHeader = if ($CutlassRoot) { Join-Path $CutlassRoot "include/cute/tensor.hpp" } else { "" }

if ($cutlassHeader -and (Test-Path $cutlassHeader)) {
    Write-Host "CUTLASS headers: found"
} else {
    Write-Host "CUTLASS headers: missing include/cutlass/cutlass.h"
}
